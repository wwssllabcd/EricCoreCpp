...
estring StringUtility::arrayToString(eu8_p charArray, int length, bool isEndWhenZero) {
    estring  str, temp;
...
estring StringUtility::arrayToHexString(eu8_p charArray, int length, const estring& filler) {
    estring  str, temp;
    int s;
    for (s = 0; s < length; s++) {
...
estring StringUtility::makeAsciiTable(eu8_p ary, int length) {
    estring msg;
    int width = 0x10;
    int row = 0;
...
estring StringUtility::makeHexTable(int length, eu8_p ary, eu16 offset, bool needHeader) {
    estring msg;
    estring head;
    if (needHeader) {
...
eu32 StringUtility::getFileSize(const estring& filePath) {
...
eu8_p StringUtility::getFileDataNew(const estring& filePath, int length, eu8_p data) {
    fstream file;
...
eu8_p StringUtility::getFileData(const estring& filePath, int startAddr, int length, eu8_p data) {
    if (startAddr == 0) {
...
eu8_p StringUtility::getFileData(const estring& filePath, eu32 fileSize, eu8_p data) {
    getFileData(filePath, 0, fileSize, data);
    return data;
}

eu8_p StringUtility::getFileData(const estring& filePath, eu8_p data) {
    long fileSize = getFileSize(filePath);
...
// output to Text file
void StringUtility::toFile(const estring& filePath, const estring& msg, bool isAppend) {
    toFile(filePath, (eu8*)msg.c_str(), (int)msg.length(), isAppend);
}

// output Array to Binary file
void StringUtility::toFile(const estring& filePath, eu8_p data, int length, bool isAppend) {
...
// output Array to Binary file
FILE* StringUtility::getFilePtr(const estring& filePath, const estring& condition) {
    FILE* fpSource = 0;
    tfopen_s(&fpSource, filePath.c_str(), condition.c_str());
    return fpSource;
}

bool StringUtility::isFileExist(const estring& filePath) {
    struct _stat buffer;
    return (_tstat(filePath.c_str(), &buffer) == 0);
}

int StringUtility::deleteFile(const estring& filePath) {
    return _tremove(filePath.c_str());
}

void StringUtility::createFolder(const estring& folderName) {
...
int StringUtility::toInt(const bool& boolean) {
    int intTemp;
    if (boolean) {
        intTemp = 1;
    } else {
...
int StringUtility::toInt(eu8 value) {
    return (int)value;
}
int StringUtility::getBit(const eu16& word, int bitNo) {
    //using Bitset of STD(C++ Primer 3rd Edition ch4.11 , ch4.12)
    std::bitset<32> result(word);
    return result[bitNo];
}

int StringUtility::getBit(const eu8& byte, int bitNo) {
    // it MUST change the type or infinite loop
    eu16 target(byte);
    return getBit(target, bitNo);
}

bool StringUtility::toBool(eu32 intValue) {
    if (intValue == 0) {
        return false;
    }
    return true;
...
void StringUtility::toArray(const eu16& source, eu8_p array, bool isMSB) {
...
void StringUtility::toArray(const eu32& source, eu8_p array, bool isMSB) {
    if (isMSB == true) {
...
void StringUtility::toArray(const estring& str, eu8_p ary, int length, eu8 stuffAsciiNum) {
...
int StringUtility::ceil(int dividend, int divisor) {
...
void StringUtility::makeBuf(eu32 number, int length, eu8_p buf) {
    //align 512
    if ((length % 512) == 0) {
        eu8 wsl[512];
...
eu32 StringUtility::hexToU32(estring hex) {
    eu32 b = tstrtoul(hex.c_str(), NULL, 16);
    return b;
}

eu16 StringUtility::toU16(eu8_p ary, bool isBigEndian) {
    eu16 value = 0;
    if (isBigEndian) {
        value |= ary[0] << 8;
        value |= ary[1];
...
eu32 StringUtility::toU32(eu8_p ary, bool isBigEndian) {
    eu32 value = 0;
    if (isBigEndian) {
        value |= ary[0] << 24;
        value |= ary[1] << 16;
...
eu32 StringUtility::getCheckSum(eu8_p ary, eu32 length) {
    eu32 ckSum = 0;
...
eu32 StringUtility::getRandom(eu32 min, eu32 max) {
